...
[ ] R6: PERFORMANCE REQUIREMENTS
    [ ] R6.1: Proof generation < 60s typical / < 120s max-size;
              verification < 1s individual, < 10ms amortized (batched)
    [ ] R6.2: Runs on 32GB RAM, 4TB storage, 8-core CPU, 1 Gbps
...
```
⚠ PLACEHOLDER: literal byte values will be generated by the reference
implementation before finalization (see spec Section 18.1). Until then,
verify structure and domain separation, not specific output bytes.

h_nullifier(0x00^64):
    Domain: "Quantum-v1.nullifier"
    Output: [TO BE COMPUTED — spec Section 18.1, Test 1]

h_merkle(0x00 || leaf1 || leaf2) where leaf1 = leaf2 = LeafHash(0x00^32):
    Domain: "Quantum-v1.merkle"
    Output: [TO BE COMPUTED — spec Section 18.1, Test 3]

Domain separation test (verifiable now):
...
struct Output {
    ...
    note_commitment: [u8; 32],        // Ownership binding (spec Section 9.1)
    kyber_ciphertext: [u8; 1568],    // ML-KEM-1024
    encrypted_data: [u8; 148],        // AES-GCM: nonce(12) || ct(120) || tag(16)
}
...
V301.4: Output size matches specification (~4.9 KB, spec Section 9.1)
V301.5: note_commitment recomputable by recipient during scanning
```
...
struct Transaction {
    ...
    outputs: Vec<Output>,
    fee: u64,
    validity_proof: StarkProof,   // includes in-circuit spend authorization
    anchor: [u8; 32],
}
// No on-chain signature structure: SPHINCS+ signatures are witness data
// verified inside the STARK (spec Sections 9.3 and 10.1).
...
struct PublicInputs {
    ...
    output_note_commitments: Vec<[u8; 32]>,
    fee: u64,
    anchor: [u8; 32],
    sighash: [u8; 32],               // binds proof to tx content
}
...
struct Witness {
    ...
    input_note_blinds: Vec<[u8; 32]>,
    input_nullifier_keys: Vec<[u8; 32]>,
    input_spend_pks: Vec<[u8; 64]>,      // SPHINCS+ keys bound in notes
    input_spend_sigs: Vec<[u8; 49856]>,  // verified in-circuit
    output_values: Vec<u64>,
    output_blindings: Vec<PolyVec>,
    output_owner_data: Vec<OwnerData>,   // (SpendPKHash, NullifierPK, s)
}
```
...
An implementation is considered **complete** when all boxes below are
checked. (No implementation exists yet — these are acceptance criteria,
not status.)

```
Cryptographic Layer:
    [ ] All hash functions implemented with test vectors passing
    [ ] Lattice commitment scheme with binding/hiding tests
    [ ] ML-KEM integration with KAT vectors
    [ ] SPHINCS+ integration with KAT vectors
    [ ] Merkle tree with correctness tests

Transaction Layer:
    [ ] Output creation and encryption
    [ ] Output scanning and decryption
    [ ] Nullifier derivation
    [ ] Transaction structure serialization

Proof System:
    [ ] AIR constraints correctly specified
    [ ] STARK prover generates valid proofs
...
Consensus:
    ...
    [ ] Block validation with transaction verification
...
quantum test --vectors

# Run fuzz tests (1 hour)
quantum fuzz --duration 3600

# Run performance benchmarks
quantum bench --full

# Run security checks
quantum audit --security

# Verify against specification
quantum verify --spec ./zkprivacy-quantum-spec-v1.md

# Generate implementation report
quantum report --output implementation-report.json
```
